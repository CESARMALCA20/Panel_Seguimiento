"""
Ruta Flask para Sesiones Educativas y Demostrativas de DNT
Agrega esto a Portada.py:
    from sesiones_dnt_flask import sesiones_dnt_bp
    app.register_blueprint(sesiones_dnt_bp)
"""
...
sesiones_dnt_bp = Blueprint("sesiones_dnt", __name__)
...
# ── ITEMS CONFIG ──────────────────────────────────────────────────────────────
# Cada seccion (ALI, AFI, SBU) tiene su propio par C0009 + C0010.
# C0009: filtra Valor_Lab == ALI | AFI | SBU
# C0010: filtra Valor_Lab == "18" (igual en las 3 secciones pero columnas separadas)

ITEMS_CONFIG = [
    # Seccion ALI
    ("C0009", "C0009_ALI",    "C0009"),
    ("C0010", "C0010_18_ALI", "C0010"),
    # Seccion AFI
    ("C0009", "C0009_AFI",    "C0009"),
    ("C0010", "C0010_18_AFI", "C0010"),
    # Seccion SBU
    ("C0009", "C0009_SBU",    "C0009"),
    ("C0010", "C0010_18_SBU", "C0010"),
]
N_ITEMS = len(ITEMS_CONFIG)

# Grupos para la doble cabecera de la tabla (titulo, n_cols, color_hex, [id_cols])
GRUPOS = [
    ("SESION EDUCATIVA Y DEMOSTRATIVA EN ALIMENTACION SALUDABLE (ALI)",          2, "#1e40af", ["C0009_ALI", "C0010_18_ALI"]),
    ("SESION EDUCATIVA Y DEMOSTRATIVA EN ALIMENTACION SALUDABLE ACTIVIDAD FISICA (AF)", 2, "#065f46", ["C0009_AFI", "C0010_18_AFI"]),
    ("SESION EDUCATIVA Y DEMOSTRATIVA EN SALUD BUCAL (SBU)",                     2, "#7c2d12", ["C0009_SBU", "C0010_18_SBU"]),
]
...
_SECCION_VLAB = {"ALI": "ALI", "AFI": "AFI", "SBU": "SBU"}


def extraer_c0009(df: pl.DataFrame, vlab_seccion: str) -> dict:
    """Extrae C0009 para una seccion (ALI/AFI/SBU).
    Retorna {dni: "dd/mm/yyyy (VLAB)"} con la fecha mas reciente.
    """
    sub = df.filter(
        (pl.col("Codigo_Item") == "C0009") &
        (pl.col("Valor_Lab").str.to_uppercase() == vlab_seccion.upper())
    )
    if sub.is_empty():
        return {}
    sub = sub.sort("Fecha_Atencion", descending=True)
    grp = sub.group_by("Numero_Documento_Paciente").agg([
...
def extraer_c0010_por_seccion(df: pl.DataFrame, vlab_seccion: str,
                               fechas_c0009: dict) -> dict:
    """Extrae C0010 (Valor_Lab=18) vinculado a la seccion por fecha.

    Logica: para cada paciente, el C0010 valido es aquel cuya Fecha_Atencion
    coincide con la fecha del C0009 de la misma seccion.
    Si no hay coincidencia exacta de fecha, la celda queda vacia (no realizado).

    fechas_c0009: {dni: "dd/mm/yyyy (VLAB)"} — resultado de extraer_c0009
    """
    sub = df.filter(
        (pl.col("Codigo_Item") == "C0010") &
        (pl.col("Valor_Lab").str.to_uppercase() == "18")
    )
    if sub.is_empty():
        return {}

    # Construir mapa dni -> set de fechas del C0010 disponibles
    sub = sub.sort("Fecha_Atencion", descending=True)
    grp = sub.group_by("Numero_Documento_Paciente").agg(
        pl.col("Fecha_Atencion").alias("fechas_c0010")
    )
    fechas_c0010_map = {}
    for row in grp.iter_rows(named=True):
        dni = str(row["Numero_Documento_Paciente"])
        # set de fechas formateadas dd/mm/yyyy
        fechas_c0010_map[dni] = {
            f.strftime("%d/%m/%Y") for f in row["fechas_c0010"] if f is not None
        }

    resultado = {}
    for dni, val_c0009 in fechas_c0009.items():
        # val_c0009 = "dd/mm/yyyy (VLAB)" o "dd/mm/yyyy"
        fecha_seccion = val_c0009[:10]  # primeros 10 chars = "dd/mm/yyyy"
        fechas_disponibles = fechas_c0010_map.get(dni, set())
        if fecha_seccion in fechas_disponibles:
            resultado[dni] = f"{fecha_seccion} (18)"
        # si no coincide fecha, no se agrega -> quedara como NaN -> "❌"
    return resultado
...
def _get_df():
    """Obtiene el DataFrame con C0009/C0010 desde el caché compartido."""
    global _DF_FILTRADO
    ...
    try:
        df = df_raw.filter(
            pl.col("Codigo_Item").is_in(["C0009", "C0010"])
        )
...
def procesar_datos(ipress_sel, mes_sel, dni_raw, anio=2026):
    try:
        ...
        # ── Num_Pag y Num_Reg: cascada ALI → AFI → SBU del C0009 más reciente ──
        def _extraer_num_map(df_src, vlab):
            """Retorna {dni: (Num_Pag, Num_Reg)} del C0009/vlab más reciente."""
            sub = df_src.filter(
                (pl.col("Codigo_Item") == "C0009") &
                (pl.col("Valor_Lab").cast(pl.Utf8).str.to_uppercase() == vlab.upper())
            ).filter(
        ...
        _map_ali = _extraer_num_map(df, "ALI")
        _map_afi = _extraer_num_map(df, "AFI")
        _map_sbu = _extraer_num_map(df, "SBU")

        def _get_num(dni):
            """Cascada: usa ALI, si no AFI, si no SBU."""
            return _map_ali.get(dni) or _map_afi.get(dni) or _map_sbu.get(dni) or ("", "")

        # ── Lote: el más frecuente entre todos los registros C0009/C0010 ──
        # ── Demás campos: del registro C0009/C0010 más reciente ──
        df_sorted = df.filter(
            pl.col("Numero_Documento_Paciente").cast(pl.Utf8).str.strip_chars().str.len_chars() > 0
        ).sort("Fecha_Atencion", descending=True)
        ...
        # Extraer C0009 por seccion
        c0009_ali = extraer_c0009(df, "ALI")
        c0009_afi = extraer_c0009(df, "AFI")
        c0009_sbu = extraer_c0009(df, "SBU")

        # Extraer C0010 vinculado por fecha a cada seccion
        c0010_ali = extraer_c0010_por_seccion(df, "ALI", c0009_ali)
        c0010_afi = extraer_c0010_por_seccion(df, "AFI", c0009_afi)
        c0010_sbu = extraer_c0010_por_seccion(df, "SBU", c0009_sbu)

        resultado_items = {
            "C0009_ALI":    c0009_ali,
            "C0010_18_ALI": c0010_ali,
            "C0009_AFI":    c0009_afi,
            "C0010_18_AFI": c0010_afi,
            "C0009_SBU":    c0009_sbu,
            "C0010_18_SBU": c0010_sbu,
        }

        df_final = df_info.to_pandas()
        df_final["Numero_Documento_Paciente"] = df_final["Numero_Documento_Paciente"].astype(str)

        # Asignar Num_Pag y Num_Reg con cascada ALI → AFI → SBU
        _tiene_num = bool(_map_ali or _map_afi or _map_sbu)
        if _tiene_num:
        ...
        columnas_indicadores = ids_pivot  # ["C0009_ALI","C0010_18_ALI","C0009_AFI",...]
        ...
        # Solo mostrar pacientes con AL MENOS un C0009/C0010 realizado
...
def render_page(df_final, columnas_indicadores, lista_ipress, lista_meses,
    ...
    # Fila 1: celdas fijas rowspan=2 + cabeceras de grupo colspan=2
    _opc_hdrs = []
    if df_final is not None and not df_final.empty:
    ...
    tr1  = '<th class="s-idx" rowspan="2">#</th>'
    tr1 += '<th class="s-dni" rowspan="2">DNI</th>'
    tr1 += '<th class="s-pac" rowspan="2">Paciente</th>'
    for h in fixed_headers:
        tr1 += f'<th rowspan="2">{h}</th>'
    for titulo, ncols, color, _ in GRUPOS:
        tr1 += (
    ...
    # Fila 2: sub-columnas de cada grupo (C0009 / C0010)
    tr2 = ""
    for _, _, color, id_cols in GRUPOS:
        for id_col in id_cols:
            label = next((lbl for _cod, idc, lbl in ITEMS_CONFIG if idc == id_col), id_col)
            tr2 += (
                f'<th style="background:{color};color:#fff;opacity:.88;'
                f'border-left:1px solid rgba(255,255,255,.3);font-size:.6rem;">'
                f'{label}</th>'
            )

    ths = f"<tr>{tr1}</tr><tr>{tr2}</tr>"

        # ── HTML completo (mismo diseño que joven_flask.py) ───────────────────────
    html = f"""<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Red San Pablo — Sesiones Educativas DNT</title>
...
table{{border-collapse:separate;border-spacing:0;font-size:.72rem;width:100%;}}
tbody td{{white-space:nowrap;}}
thead tr:first-child{{background:linear-gradient(135deg,#0a1a5c,#1C398E);color:#fff;position:sticky;top:0;z-index:21;}}
thead tr:last-child{{background:linear-gradient(135deg,#0f2472,#1C398E);color:#fff;position:sticky;top:47px;z-index:20;}}
thead th{{padding:6px 10px;font-weight:700;font-size:.63rem;letter-spacing:.04em;text-transform:uppercase;border-bottom:2px solid rgba(255,255,255,.5);border-top:1px solid rgba(255,255,255,.3);border-right:1px solid rgba(255,255,255,.25);white-space:normal;word-break:keep-all;min-width:80px;}}
...
thead th[colspan]{{white-space:normal;text-align:center;min-width:160px;}}
...
.rsp-ld-logo img{{width:50px;height:50px;object-fit:contain;mix-blend-mode:screen;}}
.rsp-ld-brand{{text-align:center;}}
.rsp-ld-brand b{{display:block;font-size:.92rem;font-weight:800;color:#0b1e42;letter-spacing:-.01em;}}
.rsp-ld-brand small{{display:block;font-size:.72rem;color:#5a7a96;margin-top:2px;}}
.rsp-spinner{{
...
.rsp-ld-txt{{font-size:.78rem;color:#5a7a96;font-weight:500;}}
.rsp-ld-bar{{width:180px;height:3px;background:#c8d8e8;border-radius:2px;overflow:hidden;}}
.rsp-ld-fill{{
  height:100%;width:0%;
  background:linear-gradient(90deg,#1C398E,#2563eb);
  border-radius:2px;
  animation:rsp-bar 2s ease-in-out infinite;
}}
@keyframes rsp-bar{{0%{{width:0%}}60%{{width:75%}}100%{{width:100%}}}}
</style>
</head>
<body>
...
<div class="topbar">
  <div class="topbar-left">
    ...
    <div>
      ...
      <div class="topbar-sub">Seguimiento de Sesiones Educativas y Demostrativas de DNT</div>
    </div>
  </div>
...
<div class="content">
  <!-- HERO -->
  <div class="hero">
    <h1>SESIONES EDUCATIVAS Y DEMOSTRATIVAS <span>DNT</span></h1>
  ...
  <!-- FILTERS -->
  <form method="GET" action="/sesiones-dnt">
    <div class="filters">
      ...
      <div class="filter-actions">
        ...
        <a href="/sesiones-dnt" class="btn-clear">
  ...
  {'<p style="color:#64748b;font-size:.8rem;margin-top:1rem;text-align:center;">Configure los filtros para visualizar los datos.</p>' if (df_final is None or df_final.empty) and not aviso and not error else ''}
...
function descargarExcel() {{
    ...
    ws['!cols'] = colWidths;
    XLSX.utils.book_append_sheet(wb, ws, 'SesionesDNT');
    var fecha = new Date();
    var nombreArchivo = 'SesionesDNT_' + fecha.getFullYear() + ('0'+(fecha.getMonth()+1)).slice(-2) + ('0'+fecha.getDate()).slice(-2) + '.xlsx';
...
@sesiones_dnt_bp.route("/sesiones-dnt")
def sesiones_dnt():
    if not request.args:
        from flask import redirect, url_for
        return redirect(url_for("sesiones_dnt.sesiones_dnt", ipress=IPRESS_DEFAULT))
